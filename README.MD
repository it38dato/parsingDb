...
<p>
    ...
    <strong>Decision:</strong><br>
    проект находится в папке Projects (parsingEtlWarehouse)<br>
...
<p>
    <strong>Task:</strong><br>
    Написать скрипт, который переносит данные с формата txt в БД Postgresql<br>
    <strong>Decision:</strong><br>
    проект находится в папке Projects (convertingTxtPsql)<br>
    Перед запуском проекта необходима библиотека psycopg2-binary<br>
    файл textDictionary.txt должен находиться рядом с файлом main.py
</p>